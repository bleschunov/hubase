...
def _main(csv_options: CsvOptions, logger: Logger) -> t.Iterator[CSVRow]:
    ...
    for url, searching_params in SearchPage(
        search_queries, logger, csv_options.max_sites_count
    ).found():
        try:
        ...
        for row in GPTCSVRows(
            people=GPTPeople(
                text=md,
                prompt_template=prompt_template,
                api_key=csv_options.openai_api_key or settings.openai_api_key,
                logger=logger,
                openai_api_base=csv_options.openai_api_base
            ...
            yield row
            if csv_options.mode == "researcher":
                # Process next site
                break
    ...
    with HubaseCsv(headers=headers, settings=settings) as csv_:
        ...
        for lead_count, person in enumerate(
            ...
            if lead_count >= csv_options.max_lead_count:
                logger.info(
                    "Достигнуто максимальное установленное количество лидов"
                )
                break
